...
async function cordovaReady({ lib, game, get, _status, ui }) {
  lib.path = (await __vitePreload(async () => {
    const { default: __vite_default__ } = await import("../../vendor/pnpm/path-browserify-esm@1.0.6/modules/path-browserify-esm/index.esm.js");
  ...
  game.ensureDirectory = (list, callback, file) => {
    const directoryList = typeof list == "string" ? [list] : list.slice().reverse(), num = file ? 1 : 0, access = (entry, directory, createDirectory) => {
      ...
      return new Promise(
        (resolve, reject) => entry.getDirectory(
          str,
          {
            create: false
          },
    ...
    return new Promise(
      (resolve, reject) => window.resolveLocalFileSystemURL(
        nonameInitialized,
        (rootEntry) => {
