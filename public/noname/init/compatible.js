import "../../noname.js";
import compiler from "../library/element/GameEvent/compilers/ContentCompiler.js";
...
lib.element.Player.prototype.when = function(...triggerNames) {
  ...
  let skill = {
    ...
    get filter() {
      return (event, player2, name) => skill.filterFuns.every((fun) => Boolean(fun(event, player2, name))) && skill.filter2(event, player2, name);
    },
    get filter2() {
      return (event, player2, name) => skill.filter2Funs.length === 0 || skill.filter2Funs.some((fun) => Boolean(fun(event, player2, name)));
    }
  };
